...
// 게시글 조회페이지
router.get("/article/:_id", async (req: Request, res: Response) => {
...
// 삭제기능
router.delete("/article/:_id", async (req: Request, res: Response) => {
  const { _id } = req.params;
...
// 수정
router.put("/article/:_id", async (req: Request, res: Response) => {
  const { _id } = req.params;
  ...
  if (article.password != password) {
    ...
    await Article.updateOne(
      { _id },
      { $set: { password, title, content, name } }
    );
...
// 생성 파일
router.post("/article", authMiddlleware, async (req: Request, res: Response) => {
